// 급여명세서 발급 (샘플/모의 계산 - 실제 급여 데이터가 DB에 없어 발급 시 입력한 금액으로 계산합니다)
import React, { useEffect, useMemo, useState } from 'react';
import { useAuth } from '../../context/AuthContext';
import { supabase } from '../../supabaseClient';
import Loading from '../Loading';
import ErrorMessage from '../ErrorMessage';
import PayslipTemplate, { type PayslipLine } from '../../pages/document-template/PayslipTemplate';
...
const PayslipManager: React.FC = () => {
  ...
  const [payMonth, setPayMonth] = useState(getThisMonthKey());
  ...
  const canGenerate = !!selectedUser && baseSalary > 0;

  const { payments, deductions } = useMemo(() => {
    const gross = baseSalary + positionAllowance + mealAllowance;
    ...
    // 아래 소득세/지방소득세는 실제 세율표가 아닌 단순화된 샘플 계산입니다.
    const incomeTax = Math.round(gross * 0.03);
    const localIncomeTax = Math.round(incomeTax * 0.1);

    const payments: PayslipLine[] = [
      { label: '기본급', amount: baseSalary },
      { label: '직책수당', amount: positionAllowance },
      { label: '식대(비과세)', amount: mealAllowance },
    ];
    ...
    const deductions: PayslipLine[] = [
      ...
      { label: '소득세(샘플)', amount: incomeTax },
      { label: '지방소득세(샘플)', amount: localIncomeTax },
    ];

    return { payments, deductions };
  }, [baseSalary, positionAllowance, mealAllowance]);
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 space-y-4">
      <div className="no-print">
        <h1 className="text-2xl font-bold">급여명세서 발급 (샘플)</h1>
        <p className="text-sm text-gray-600 mt-1">
          실제 급여 데이터가 DB에 없어, 아래 입력한 금액을 기준으로 간이 계산합니다. 법적 효력이 없는 샘플 문서입니다.
        </p>
      </div>
      ...
      <div className="no-print bg-white shadow rounded-lg p-4 space-y-3">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-800 mb-1">지급 월</label>
            <input
              type="month"
              value={payMonth}
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-800 mb-1">기본급 (원) *</label>
            <input
              type="number"
              min={0}
              value={baseSalary}
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-800 mb-1">직책수당 (원)</label>
            <input
              type="number"
              min={0}
          ...
          <div>
            <label className="block text-sm font-semibold text-gray-800 mb-1">식대(비과세, 원)</label>
            <input
              type="number"
              min={0}
              value={mealAllowance}
        ...
        {!canGenerate && <p className="text-xs text-red-600">직원 선택과 기본급 입력(0보다 커야 함)이 필요합니다.</p>}
      ...
      {showPreview && selectedUser && (
        <PayslipTemplate
          issueDate={issueDate}
          payMonthText={`${payMonth.replace('-', '년 ')}월`}
