  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-medium text-gray-900">휴가 결재 업무</h2>
        <button
          onClick={fetchQueue}
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {rows.map((r) => (
                <tr key={r.leave_approval_id}>
                  ...
                  <td className="px-4 py-2.5 whitespace-nowrap text-xs text-gray-700">{r.days_requested}일</td>
      ...
      <div className="pt-2">
        <h3 className="text-sm font-medium text-gray-900 mb-2">내가 결재/참조로 지정된 휴가 신청 내역</h3>
              ...
              <table className="min-w-full divide-y divide-gray-100">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-4 py-2 text-center text-xs font-medium text-gray-500">일수</th>
                    <th className="px-4 py-2 text-center text-xs font-medium text-gray-500">사유</th>
                    <th className="px-4 py-2 text-center text-xs font-medium text-gray-500">내 역할</th>
                ...
                <tbody className="bg-white divide-y divide-gray-100">
                  {visRows.map((r) => (
                    <tr key={r.leave_id}>
                      <td className="px-4 py-2.5 whitespace-nowrap text-xs text-gray-700">{displayVisRequester(r.requester_user_id)}</td>
                      <td className="px-4 py-2.5 whitespace-nowrap text-xs text-gray-700">
                        {r.start_date} ~ {r.end_date}
                      </td>
                      <td className="px-4 py-2.5 whitespace-nowrap text-xs text-gray-700">{r.days_requested}일</td>
                      ...
                      <td className="px-4 py-2.5 whitespace-nowrap text-xs text-gray-700">
                        ...
                        {r.is_current_turn && (
                          <span className="ml-1.5 px-1.5 py-0.5 rounded text-xs bg-blue-100 text-blue-700">내 차례</span>
                        )}
                      </td>
                      <td className="px-4 py-2.5 whitespace-nowrap text-xs text-gray-700">
