...
const ReactionEmojiManager: React.FC = () => {
    ...
    const invalidateEmojiCache = () => {
        if (typeof window === 'undefined') return;
        try {
            window.localStorage.removeItem('reaction_emojis_active_v1');
        } catch {
            // ignore
        }
        window.dispatchEvent(new Event('reaction-emojis-updated'));
    };
                                    ...
                                    <li key={it.id} className="p-3 flex items-center justify-between gap-3">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <button
                                                ...
                                                className={`px-2 py-1 rounded-md text-xs border ${it.is_active
