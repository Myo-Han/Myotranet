// 연차 신청/수정 폼 본문 (공용 컴포넌트).
// pages/LeaveRequestPage.tsx(풀페이지: /leave/new, /leave/edit/:leaveId)와
// LeaveAnnualPanel.tsx(근태관리 탭 안에서의 인라인 신청/수정)가 이 컴포넌트를 함께 쓴다.
// 페이지 제목/취소링크 같은 "바깥 껍데기"는 각 호출부가 알아서 그리고, 이 컴포넌트는
// 폼 필드 + 결재라인 빌더 + 제출/삭제/취소 버튼만 담당한다. 완료(제출 성공/삭제 성공/취소)
// 시점에는 navigate 대신 onDone() 콜백을 호출해서, 호출부가 풀페이지 이동이든 인라인 닫기든
// 원하는 방식으로 처리하게 한다.
//
...
const LeaveRequestFormBody: React.FC<Props> = ({ leaveId, onDone }) => {
  const { user } = useAuth();
    ...
    <div className="space-y-4">
        ...
        <div className="bg-white rounded-md border border-gray-200 p-4 space-y-3">
          {lr.balancePoolLabel && (
            <div className="text-sm bg-blue-50 border border-blue-100 rounded-md px-3 py-2 text-blue-800">
              현재 {lr.balancePoolLabel} 잔여일수: <span className="font-medium">{lr.availableBalance}일</span>
            </div>
          )}
      ...
      <div className="flex gap-2 pb-2">
        <button
          ...
          className="flex-1 px-4 py-2 bg-blue-600 text-white text-sm rounded-md hover:bg-blue-700 disabled:opacity-40"
        >
...
export default LeaveRequestFormBody;
