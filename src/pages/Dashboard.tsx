...
const HiddenBadge = ({ hidden }: { hidden: boolean }) =>
  hidden ? (
    <span
      className="ml-1 inline-flex shrink-0 items-center gap-0.5 rounded-full bg-gray-100 px-1.5 py-0.5 text-[10px] font-medium text-gray-500"
...
const Dashboard: React.FC = () => {
  ...
  // ✅ 필드별 공개 범위(field_visibility) - 관리자 전용(false)으로 설정된 항목을
  // 관리자 본인 화면에서는 "비공개" 배지로 표시해줘서, 일반 직원에게는 이 항목이
  // 보이지 않는다는 걸 관리자도 시각적으로 확인할 수 있게 함.
  // (관리자/본인은 UserManager.tsx의 FIELD_VISIBILITY_DEFAULTS와 동일한 기본값 사용)
  ...
  // ✅ 우측 상단 검색창 제출 시: 검색어를 그대로 SearchModal에 넘기고 모달을 열어 검색을 실행시킴
  ...
  return (
    <div className="space-y-6">
      {/* ✅ 우측 상단 검색창 (기존 "직원 검색" 카드 대체) */}
      <div className="flex justify-end">
        <form onSubmit={handleHeaderSearchSubmit} className="relative w-full max-w-xs">
          <svg
          ...
          <input
            ...
            className="w-full rounded-full border border-gray-300 bg-white py-2 pl-9 pr-4 text-sm shadow-sm focus:border-indigo-400 focus:outline-none focus:ring-1 focus:ring-indigo-400"
          />
        </form>
      </div>
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-3 space-y-1.5">
                    <div className="flex items-center gap-2 text-sm text-gray-600" title="사번">
                      ...
                      {user?.role === 'Admin' && <HiddenBadge hidden={isHiddenFromOthers('employee_no')} />}
                    </div>
                    <div className="flex items-center gap-2 text-sm text-gray-600" title="소속">
                      ...
                      {user?.role === 'Admin' && <HiddenBadge hidden={isHiddenFromOthers('affiliation')} />}
                    </div>
                    <div className="flex items-center gap-2 text-sm text-gray-600" title="이메일">
                      ...
                      {user?.role === 'Admin' && <HiddenBadge hidden={isHiddenFromOthers('email')} />}
                    </div>
                    <div className="flex items-center gap-2 text-sm text-gray-600" title="휴대폰 번호">
                      ...
                      {user?.role === 'Admin' && <HiddenBadge hidden={isHiddenFromOthers('phone')} />}
                    </div>
                    <div className="flex items-center gap-2 text-sm text-gray-600" title="생일">
                      ...
                      {user?.role === 'Admin' && <HiddenBadge hidden={isHiddenFromOthers('birth_date')} />}
                    </div>
                    <div className="flex items-center gap-2 text-sm text-gray-600" title="입사일">
                      ...
                      {user?.role === 'Admin' && <HiddenBadge hidden={isHiddenFromOthers('hire_date')} />}
                    </div>
                    <div className="flex items-center gap-2 text-sm text-gray-600 italic" title="상태 메시지">
                      ...
                      {user?.role === 'Admin' && <HiddenBadge hidden={isHiddenFromOthers('status_message')} />}
                    </div>
                  </div>
                </div>
              </div>
